 ...
 *
 * @link       https://github.com/AgriLife/agrilife-livewhale/blob/master/src/class-widgets.php
 * @since      0.1.5
...
class Widget_LiveWhale extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'agrilife_livewhale', __( 'LiveWhale' ), $widget_ops, $control_ops );
	...
	public function widget( $args, $instance ) {
		...
		$feed_json = wp_remote_get( $feed_url );
	...
	public function form( $instance ) {
		...
		$feed_url_output = '<p><label for="%s">%s</label><input type="text" id="%s" name="%s" class="feed_url widefat" value="%s"/></p>';
		...
		echo wp_kses(
			sprintf(
				...
				esc_attr( 'Direction:', 'agrilife-livewhale' ),
				esc_attr( $this->get_field_id( 'direction' ) ),
